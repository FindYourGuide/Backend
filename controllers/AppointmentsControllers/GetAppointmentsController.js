...
async function GetAvailableAppointmentsByDate(req, res) {
  const userId = req.user._id;
  const { date, counselorId } = req.body;
  try {
    const targetDate = new Date(date);
    ...
    if (appointments) {
      return responseMessage(res, 200, "Slots for counselor by date", appointments)
    }
    else {
      return responseMessage(res, 200, "No Slots for counselor by date")
    }
